create view ConsumosEstadia
as
    select Mascotas.CodMascota, Estadias.CheckIn, TipoServ, Cantidad, Cargo
    from Servicios
...
BEGIN
    ...
    set @ver = (select CheckOut
    from Estadias
    where CheckIn=@CheckIn and CodMascota=@CodMascota and NroHab=@NroHab)
    if @ver is NULL
        BEGIN
        declare CursorConsumos cursor for select CheckIn, CodMascota, TipoServ, Cantidad, Cargo
        from ConsumosEstadia
        ...
        @Cargo money,
        @Sumador money = 0,
        @Alias varchar (20) = (select Alias
        from Mascotas
        where CodMascota = @CodMascota),
        @PrecioHotel money = (select Dias
        from Estadias
        where CheckIn=@CheckIn and CodMascota=@CodMascota and NroHab=@NroHab) * 60,
        @Dias char (2) = (select Dias
        from Estadias
        where CheckIn=@CheckIn and CodMascota=@CodMascota and NroHab=@NroHab)
...
BEGIN
    ...
    if @ver is NULL
        ...
        from Estadias
        where CheckIn=@CheckIn and CodMascota=@CodMascota and NroHab=@NroHab)
        set @PrintText= 'La mascota '+@Alias+ ' a fecha de hoy, '+CONVERT(varchar (10), @CheckOut)+ ' realiza su Check Out de nuestro hotel.';insert into @PrintTable
        values
            (@PrintText)
        set @PrintText= '';insert into @PrintTable
        values
            (@PrintText)
        set @PrintText= 'Por la estadía de '+@Dias+ 'dias en el hotel se genera un total de: Bs. '+convert(varchar(10),@PrecioHotel);insert into @PrintTable
        values
            (@PrintText)
        set @PrintText= '';insert into @PrintTable
        values
        ...
        where CheckIn = @CheckIn and CodMascota=@CodMascota and NroHab=  @NroHab)
            BEGIN
            set @PrintText= 'Ademas, habiendo solicitado los siguientes servicios:';insert into @PrintTable
            values
                (@PrintText)
            set @PrintText= '';insert into @PrintTable
            values
            ...
            values
                (@PrintText)
            set @PrintText= '|  SERVICIO     -     CANTIDAD     -     CARGO  |';insert into @PrintTable
            values
                (@PrintText)
            set @PrintText= '―――――――――――――――――――――――――――――――――――――――――――――――――';insert into @PrintTable
            values
        ...
        ELSE
            begin
            set @PrintText='No habiendo solicitado ningún servicio adicional';insert into @PrintTable
            values
                (@PrintText)
            set @PrintText='';insert into @PrintTable
            values
        ...
        values
            (@PrintText)
        set @PrintText='Generándose un total de Bs. '+convert(varchar(10),(@PrecioHotel+@Sumador))+' por la estadía en el hotel';insert into @PrintTable
        values
            (@PrintText)
        set @PrintText =''
        set @PrintText='Habiento obtenido '+ convert(char(1),floor(convert(int,@Dias)/7)+1) + ' baño(s) de cortesía';insert into @PrintTable
        values
            (@PrintText)
        set @PrintText='';insert into @PrintTable
        values
            (@PrintText)
        set @PrintText='Atte. Hotel Happy Fox';insert into @PrintTable
        values
            (@PrintText)
    END
    ELSE   
        BEGIN
        set @PrintText= 'Ya se ha realizado este Checkout';insert into @PrintTable
        values
            (@PrintText)
    end
    SELECT PrintText

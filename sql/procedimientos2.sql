...
alter procedure RegistrarPeso @FechaPeso date,
@CodMascota char(5),
@Peso varchar(5),
@Check bit out as BEGIN begin TRANSACTION if exists (
    select 1
...
values (@FechaPeso, @CodMascota, CAST(@Peso AS float))
END
END
GO --Modificar un registro de peso
    alter procedure ModificarPeso @FechaPeso date,
    @CodMascota char (5),
    @NuevoPeso float,
    @Check bit out AS BEGIN begin TRANSACTION
DECLARE @SQL NVARCHAR(MAX);
update HistorialesPeso
set Peso = @NuevoPeso
where CodMascota = @CodMascota
    and FechaPeso = @FechaPeso if exists (
        select 1
        from HistorialesPeso
        where FechaPeso = @FechaPeso
            and CodMascota = @CodMascota
...
alter procedure EliminarRegPeso @FechaPeso date,
@CodMascota char (5),
@Cuenta int,
@Check bit out AS BEGIN begin TRANSACTION
...
from HistorialesPeso
where FechaPeso = @FechaPeso
    and CodMascota = @CodMascota if @Cuenta = 0 BEGIN
set @Check = 1
SELECT @Check
END
else BEGIN
...
alter PROCEDURE RegistrarVisitaMedica @FechaConsulta date,
@CodMascota char(5),
@Situacion varchar(20),
@DetalleMed varchar(40),
@Check bit out as BEGIN if exists (
    select 1
...
alter PROCEDURE ModificarVisitaMed @FechaConsulta DATE,
@CodMascota CHAR(5),
@Campo VARCHAR(20),
...
alter procedure EliminarVisitaMed @FechaConsulta date,
@CodMascota char(5),
@Cuenta int,
@Check bit out AS BEGIN begin TRANSACTION
...
alter PROCEDURE RegistrarVacunacion @FechaVacuna date,
@CodMascota char (5),
...
alter procedure EliminarVacunacion @FechaVacuna date,
@CodMascota char (5),
@TipoVacuna varchar(15),
@Fabricante varchar (20),
@Cuenta int,
...
where FechaVacuna = @FechaVacuna
    and CodMascota = @CodMascota
    and TipoVacuna = @TipoVacuna
    and Fabricante = @Fabricante if @Cuenta = 0 BEGIN
set @Check = 1
SELECT @Check
end
else begin
...
alter procedure RegistrarEstadia @CheckIn date,
@CodMascota char (5),
@NroHab char (2),
@Dias int,
@Check bit out AS BEGIN begin TRANSACTION if exists(
...
values (@CheckIn, @CodMascota, @NroHab, NULL, @Dias)
update Habitaciones
set Disponible = 'O'
where NroHab = @NroHab
END
...
alter PROCEDURE ModificarEstadia @CheckIn DATE,
@CodMascota CHAR(5),
@NroHab CHAR(2),
@Campo VARCHAR(20),
...
alter procedure EliminarEstadia @CheckIn date,
@CodMascota char (5),
@NroHab char (2),
@Cuenta int,
@Check bit out AS BEGIN begin TRANSACTION
...
where CheckIn = @CheckIn
    and CodMascota = @CodMascota
    and NroHab = @NroHab if @Cuenta = 0 BEGIN
set @Check = 1
SELECT @Check
end
else begin
...
alter procedure RegistrarRequerimiento @CodMascota char (5),
@IdServicio char(3),
@CheckIn date,
@Cantidad int,
@NroHab char (2),
...
set @Factor = 1 if @Tama ño = 'M'
set @Factor = 1.15 if @Tama ño = 'G'
set @Factor = 1.30 if @Tama ño = 'XG'
set @Factor = 1.40
...
END --servicios que pueden llegar a realizarse hasta una vez por estadía
...
select *
from Requerimientos
GO --Modificacion de requerimiento
    alter procedure ModificarRequerimiento @IdServicio char (3),
    @CheckIn date,
    @CodMascota char (5),
    @NroHab char (2),
    @Campo varchar (10),
    @NuevoValor int,
    @Check bit out AS BEGIN begin TRANSACTION if @IdServicio = 'S01'
    or @IdServicio = 'S03'
...
alter procedure EliminarRequerimiento @IdServicio char(3),
@CheckIn date,
@CodMascota char(5),
@NroHab char(2),
@Check bit out AS BEGIN
declare @Cuenta int begin TRANSACTION
...
alter procedure AgregarVacuna @TipoVacuna varchar(15),
@Fabricante varchar(20),
@Precio money,
@Check bit out AS BEGIN begin TRANSACTION if exists (
    select 1
...
SELECT @Check
insert into Vacunas
values (@TipoVacuna, @Fabricante, @Precio)
END
END
GO;
--Eliminar vacuna
alter procedure EliminarVacuna @TipoVacuna varchar(15),
@Fabricante varchar (20),
@Cuenta int,
@Check bit out AS BEGIN begin TRANSACTION
delete from Vacunas

...
BEGIN
    ...
    where FechaConsulta=@FechaConsulta and CodMascota=@CodMascota)
        ...
        values
            (@FechaConsulta, @CodMascota, @Situacion, @DetalleMed)
        set @Check = 1
        SELECT @Check
    END
END
...
BEGIN
    ...
    IF EXISTS (SELECT 1
    FROM HistorialesMedicos
    WHERE FechaConsulta = @FechaConsulta AND CodMascota = @CodMascota AND ' + QUOTENAME(@Campo) + ' = @NuevoValor)
    BEGIN
        SET @Check = 1;
        SELECT @Check;
    END
...
BEGIN
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1
        FROM Estadias
        WHERE CheckIn = @CheckIn AND CodMascota = @CodMascota AND NroHab = @NroHab AND ' + QUOTENAME(@Campo) + ' = @NuevoValor)
        BEGIN
            SET @Check = 1;
            SELECT @Check;
        END
...
as
    select CodMascota, Tamaño
    from Mascotas
GO;

select *
from TamañoMascotas
declare CursorTamaños cursor for select *
from TamañoMascotas 
GO;
...
select *
from ServiciosEsp
declare CursorServiciosEsp cursor for select *
from ServiciosEsp 
GO;
...
BEGIN
    ...
    --los demás servicios que se pueden realizar varias veces y se cobran de acuerdo al tamaño, cantidad de veces
        ...
        values
            (@IdServicio, @CheckIn, @CodMascota, @NroHab, @Cantidad, @Cargo)

        --son servicios especiales y no tienen cantidad, se cobra por la cantidad de dias
        if @IdServicio = 'S04' or @IdServicio = 'S05'
            BEGIN
            set @Cargo = (select Precio
            from Servicios
            where IdServicio = @IdServicio)
            set @Cargo *= @Dias
            insert into Requerimientos
            values
                (@IdServicio, @CheckIn, @CodMascota, @NroHab, NULL, @Cargo)
        END
        ...
        if @IdServicio = 'S02'
            ...
            insert into Requerimientos
            values
                (@IdServicio, @CheckIn, @CodMascota, @NroHab, 1, @Cargo)
        END
        else
        --los demás servicios que se pueden realizar varias veces y se cobran de acuerdo al tamaño, cantidad de veces
            BEGIN
...
BEGIN
    ...
    if exists (select 1
    from Vacunas
    where TipoVacuna=@TipoVacuna and Fabricante=@Fabricante)
        begin
        set @Check = 0
        SELECT @Check
    end
